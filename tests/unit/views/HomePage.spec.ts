...
describe('HomePage.vue Filters', () => {
  ...
  it('filter blank date', () => {
    ...
    const filtered = wrapper.vm.filteringBlankDateFromFeedResults(searchResults.results);
    const results = [{
  ...
  it('filter today date', () => {
    ...
    const filtered = wrapper.vm.filteringTodayDateFromFeedResults(searchResults.results);
    const results = [{
      // eslint-disable-next-line max-len
      deck: 'Bayonetta is a "non-stop action game" from PlatinumGames.  The titular character is a witch who can use hair-based magic, as well as firearms attached to her feet, to battle fallen angels and other foes.',
      expected_release_quarter: null,
  ...
  it('remove today date', () => {
    ...
    const filtered = wrapper.vm.removingTodayDateFromFeedResults(searchResults.results);
    const results = [{
      deck: 'The third entry in the Bayonetta series comes to Nintendo Switch.',
      expected_release_quarter: null,
      expected_release_year: null,
